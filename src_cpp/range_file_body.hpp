...
// Created By Vic Zhang(VicBig at qq dot com)
//
...
#ifndef BOOST_BEAST_HTTP_RANGE_FILE_BODY_HPP
#define BOOST_BEAST_HTTP_RANGE_FILE_BODY_HPP
...
#include <cstdint>
#include <utility>
#include "vhttp_common.h" // import _UI64_MAX
...
/** The type of the @ref message::body member.
    ...
    Messages declared using `range_file_body` will have this type for
...
*/
template<class File>
class range_file_body<File>::value_type
{
	...
	std::uint64_t range_1;
	std::uint64_t range_2;
...
public:
    ...
    */
    ...
    /// Constructor
	value_type(std::uint64_t range1, std::uint64_t range2):range_1(range1), range_2(range2){}
...
*/
template<class File>
class range_file_body<File>::writer
{
    ...
    : body_(b)
{
	...
	if(b.range_1 == _UI64_MAX){
		remain_ = body_.file_size_;
	}	else{
		// We don't do anything fancy so ignore ec
		error_code ec;
		body_.file_.seek(b.range_1, ec);    


		if(b.range_2 != _UI64_MAX){
			remain_ = b.range_2 - b.range_1 + 1;
		} else{
			remain_ = body_.file_size_ - b.range_1;
		}
	}
}
...
*/
template<class File>
class range_file_body<File>::reader
{
...
public:
    ...
    // `b` is an instance of `range_file_body`.
    //
    template<bool isRequest, class Fields>
    explicit
...
operator<<(std::ostream& os, message<
    isRequest, range_file_body<File>, Fields> const& msg) = delete;
#endif
