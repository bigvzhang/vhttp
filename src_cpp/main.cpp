//
// Copyright (c) 2016-2020 Vic Zhang & Vinnie Falco
// Created by Vic Zhang(VicBig at qq dot com) based on beast example
//
...
using range_file_body = boost::beast::http::range_file_body<boost::beast::file>;
    ...
    Send&& send)
{
    ...
    if(req.target().back() == '/')
 	...
 	auto the_mime_type = mime_type(path);
    ...
    beast::error_code ec;
    range_file_body::value_type body(req_range1, req_range2);
	...
	auto mod_time = bsc::time_fmt(last_write_time(path.c_str()));
	auto curr_time = bsc::time_fmt(std::time(NULL));

	#define set_common_res_property(res){\
		res.set(http::field::server, BOOST_BEAST_VERSION_STRING);\
		res.set(http::field::content_type, the_mime_type);\
		res.set(http::field::cache_control, "max-age=0");\
		res.set(http::field::last_modified, mod_time.c_str());\
		res.set(http::field::date, curr_time.c_str());\
		res.keep_alive(req.keep_alive());\
	}
	...
	{	
		...
		if(req_range1 == _UI64_MAX){
			http::response<range_file_body> res{
			...
			http::response<range_file_body> res{
				std::piecewise_construct,
				std::make_tuple(std::move(body)),
				std::make_tuple(http::status::partial_content, req.version())};
			set_common_res_property(res);
			...
			if(req_range2 == _UI64_MAX){
				...
				res.set(http::field::content_range,
					bsc::format("bytes %llu-%llu/%llu", req_range1, req_range2, size).c_str()
				);
			}
			return send(std::move(res));
		}
	}
	#undef set_common_res_property
}
